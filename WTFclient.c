...
int add_entries(int CFD , struct FilePaths * FileHead, struct FilePaths * HashHead, char * Updatepath, struct FilePaths * VersionHead){
	struct FilePaths * Fptr = FileHead;//pointer to head of file lists
	struct FilePaths * Hptr = HashHead;//pointer to head of file lists
	struct FilePaths * Vptr = VersionHead;//pointer to head of file lists
	


//	client needs
char * client_file_version = NULL;
char * client_tag = NULL;
char * client_file_path = NULL;
char * client_hash = NULL;
char * client_buffer = (char *) malloc(sizeof(char));
char * client_word = (char *) malloc(sizeof(char));
int bytes = 0;
int match =0;
int eof =10;
int update_fd = open(Updatepath,O_RDWR);

//takes care of any things that are in clients manifest but not in server and sends them to the update file
	while(eof!=0){
		
		eof = read(CFD,client_buffer,1);
		if(eof == 0){

			break; //end of file reached
		}
		if(eof == -1 ){
		printf("could not read from clients .Manifest file\n");
		return -1;
	}
		

	if(strcmp(client_buffer, " ")==0 || strcmp(client_buffer,"\n")==0 ){


		
		if(client_tag == NULL){
		
		client_tag =(char*) malloc(strlen(client_word)+1);
		strcpy(client_tag, client_word);
		free(client_word);
		client_word = NULL;
		client_word = (char*) malloc(sizeof(char));
		continue;
		}

		if(client_file_version == NULL){
		
		//when we are in here we are at the file version of the entry so we need to store it
		client_file_version = (char*) malloc(strlen(client_word)+1);
		strcpy(client_file_version, client_word);
		free(client_word);
		client_word = NULL;
		client_word = (char*) malloc(sizeof(char));
		continue;
		}
		else if(client_file_path == NULL){
		
		client_file_path = (char *) malloc(strlen(client_word)+1);
		strcpy(client_file_path, client_word);
		free(client_word);
		client_word = NULL;
		client_word = (char*) malloc(sizeof(char));

		}
		else if(client_hash == NULL){
		client_hash = (char *) malloc(strlen(client_word)+1);
		strcpy(client_hash , client_word);
		free(client_word);
		client_word = NULL;
		client_word = (char*) malloc(sizeof(char));
			...
			//we have the file now compare all entries of the linked list to the clients path//////////////////////////////////////////////////
		while(Fptr!=NULL){
			//if the path is found in the clients manifest then it appears in the clients manifest 
			if(strcmp(Fptr->data, client_file_path)==0){
				//then we dont need to add or remove because it is found in the server and client manifest so we just break and reset Filepaths ptr to head
				Fptr = FileHead;
				free(client_tag);
				free(client_file_version);
				free(client_file_path); 
				client_tag =NULL;
				client_file_version = NULL;
		...
		if(match ==1){
			...
			//meanign that the server files did not match with any of the clients files so we have to ti is the remove case since it never appears in the server
			//if the update file doesnt exist then make it
			if(update_fd!= -1){

			int madeU = creat(Updatepath,S_IRWXU);

			if(madeU == -1){
				printf("Unable to make .Update file!\n");
				return -1;
					}	
				
				update_fd = open(Updatepath,O_RDWR);

			}
			lseek(update_fd, 0,SEEK_END);// seek to end of file and write from there

			write(update_fd, "D", strlen("D"));
			write(update_fd, " ", strlen(" "));
			write(update_fd ,client_file_version, strlen(client_file_version));
			write(update_fd, " ", strlen(" "));
			write(update_fd, client_file_path, strlen(client_file_path));
			write(update_fd, " ", strlen(" "));
			write(update_fd,client_hash , strlen(client_hash));
			write(update_fd, "\n", strlen("\n"));
			Fptr= FileHead;
			printf("D %s\n", client_file_path);
			free(client_tag);
		...
		strcat(client_word, client_buffer);
	}
...
///////////////////////////////////////////////////////////////////take care of adding case for update/////////////////////////////////////////////////////////////////////////
lseek(CFD , 0,SEEK_SET);//lseek to the start of the file
int matchedA=0;
eof =10;
while(Fptr !=NULL){
	...
	if(strcmp(client_buffer, " ")==0 || strcmp(client_buffer,"\n")==0 ){
		...
		//if not matched still need to free to go to next entry;
		free(client_tag);
		free(client_file_version);
		free(client_file_path); 
		client_tag =NULL;
	...
	if(matchedA ==0){//meaning that the file path from server was not found so we need to add it the clients manifest
	...
	//if the update file doesnt exist then make it
			...
			write(update_fd, " ", strlen(" "));
			write(update_fd ,Vptr->data, strlen(Vptr->data));
			write(update_fd, " ", strlen(" "));
			write(update_fd, Fptr->data, strlen(Fptr->data));
			write(update_fd, " ", strlen(" "));
			write(update_fd,Hptr->data , strlen(Hptr->data));
			write(update_fd, "\n", strlen("\n"));
			printf("A %s\n", Fptr->data);
...
int Compare_manifestsU(int socketfd, int CFD, int Ssize, int Csize, int bytesread, char * project_name){

char * Updatepath = (char *) malloc(strlen("./")+strlen(project_name)+ +strlen("/.Update")+1);
char * conflict_path = (char *) malloc(strlen("./")+strlen(project_name)+ +strlen("/.Conflict")+1);
//making project path
strcat(Updatepath ,"./");
strcat(Updatepath, project_name);
strcpy(conflict_path, Updatepath);//copies
...
//server needs 
char * server_file_version = NULL;
char * server_file_path = NULL;
char * server_hash =NULL; 
char * server_buffer = (char *) malloc(sizeof(char));
char * server_word = (char *) malloc(sizeof(char));
//server does not have tag
int bytes = 0;
//head of node
struct FilePaths * head = NULL;
struct FilePaths * Hhead =NULL;
struct FilePaths * Htemp = NULL;
struct FilePaths * temp = NULL;
struct FilePaths * Vhead = NULL;
struct FilePaths * Vtemp =NULL;


//tries to open up update file

int update_fd = open(Updatepath,O_RDWR);
int conflict_fd = open(conflict_path ,O_RDWR);




//this while loop will go until the manifest file that server sent ends 
while(bytesread<= Ssize){


bytes = read(socketfd, server_buffer, 1);
bytesread++;

if(bytes == -1 ){

printf("could not read from servers .Manifest file\n");
return -1;
	...
	/*
		storing file paths for clients manifest and only finding modifies and putting it into 
		update files if needed
	*/
	if(strcmp(server_buffer, " ")==0 || strcmp(server_buffer, "\n") == 0){

		if(server_file_version == NULL){
		
		//when we are in here we are at the file version of the entry so we need to store it
		server_file_version = (char*) malloc(strlen(server_word)+1);
		strcpy(server_file_version, server_word);
		free(server_word);
		server_word = NULL;
		server_word = (char*) malloc(sizeof(char));



		if(Vhead == NULL){
			Vhead  = (struct FilePaths *) malloc(sizeof(struct FilePaths));

			Vhead->data = (char*) malloc(strlen(server_file_version)+1);//allocates length of file path
			strcpy(Vhead->data ,server_file_version);//copies server file path in head data
			Vtemp = Vhead;
			Vtemp->next =NULL;
			continue;
		...
		struct FilePaths * Node = (struct FilePaths *) malloc(sizeof(struct FilePaths));
		Node->data =(char*) malloc(strlen(server_file_version)+1);
		strcpy(Node->data, server_file_version);
		Vtemp->next =Node;
		Vtemp =temp->next;
		Vtemp->next =NULL;
		continue;
		}
		else if(server_file_path == NULL){
		//if we are in here then we have gotten version for the file entry and are now filling in the file path
		server_file_path = (char *) malloc(strlen(server_word)+1);
		strcpy(server_file_path, server_word);
		free(server_word);
		server_word = NULL;
		server_word = (char*) malloc(sizeof(char));

		if(head == NULL){
			head  = (struct FilePaths *) malloc(sizeof(struct FilePaths));

			head->data = (char*) malloc(strlen(server_file_path)+1);//allocates length of file path
			strcpy(head->data ,server_file_path);//copies server file path in head data
			temp = head;
			temp->next =NULL;
			continue;
		...
		// make a new node and add it to the end of the list
		struct FilePaths * Node = (struct FilePaths *) malloc(sizeof(struct FilePaths));
		Node->data =(char*) malloc(strlen(server_file_path)+1);
		strcpy(Node->data, server_file_path);
		temp->next =Node;
		temp =temp->next;
		temp->next =NULL;
		continue;
		}
		else if(server_hash == NULL){
		//we have filled in the server path and server file version now to fill in the hash
		server_hash = (char *) malloc(strlen(server_word)+1);
		strcpy(server_hash, server_word);
		free(server_word);
		server_word = NULL;
		server_word = (char*) malloc(sizeof(char));
		if(Hhead == NULL){
			Hhead  = (struct FilePaths *) malloc(sizeof(struct FilePaths));

			Hhead->data = (char*) malloc(strlen(server_hash)+1);//allocates length of file path
			strcpy(Hhead->data ,server_hash);//copies server file path in head data
			Htemp = Hhead;
			Htemp->next =NULL;
		...
		// make a new node and add it to the end of the list
		struct FilePaths * Node = (struct FilePaths *) malloc(sizeof(struct FilePaths));
		Node->data =(char*) malloc(strlen(server_hash)+1);
		strcpy(Node->data, server_hash);
		Htemp->next =Node;
		Htemp =Htemp->next;
		Htemp->next =NULL;
		}
...
////////////this is where we have everything for the server so now we have to traverse the .////////////////////////////////////////////////////////////////////////////////////////////
		int  eof =10;
		while(eof!= 0){

			eof=  read(CFD , client_buffer, 1);
			if(eof == 0){
			//reached end of file so break;
			break;
		...
		// now we compare paths with the servers path and if they do not equal we reset everything and look at next entry

		if(strcmp(client_file_path , server_file_path) == 0){
			//if both paths are the same and we check the file versions and stored hash, if they are different and the live hash of the client
			//this writes to a Modify entry on success
			if(strcmp(client_file_version ,server_file_version) != 0 &&  strcmp(client_hash,server_hash) !=0){


			int Filefd = open(client_file_path , O_RDONLY);//open file path
			...
			stat(client_file_path,&st);

 			int size;
 
  			if (stat(client_file_path, &st) == 0) {
			...
			if(strcmp(livehash,client_hash) == 0){
			...
			//hashes match so now  we write a modify to the file
			bytes = write(update_fd , "M", strlen("M"));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}

			bytes = write(update_fd , " ", strlen(" "));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}

			bytes = write(update_fd , server_file_version, strlen(server_file_version));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}
			...
			bytes = write(update_fd,server_file_path,strlen(server_file_path));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}
			...
			bytes = write(update_fd, server_hash,strlen(server_hash));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}

			bytes = write(update_fd , "\n", strlen("\n"));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}
			...
			//free all holding variables



		}

	/////////////////////////////////////////////////////////////////////adding a Conflict entry since file paths match////////////////////////////////
	//get live hash
	int Filefd = open(client_file_path , O_RDONLY);//open file path
	...
	if(strcmp(client_hash, server_hash) !=0 && strcmp(client_hash, livehash)!=0){
		//the clients manifest file that is not equal to the servers hash or its live hash so we write a .conflict entry;
		//if the conflict file doesnt exist then make it
			if(conflict_fd!= -1){

			int madeC = creat(conflict_path,S_IRWXU);
			...
			if(madeC == -1){
				...
				conflict_fd = open(conflict_path,O_RDWR);

			}
		
		//hashes match so now  we write a modify to the file
			bytes = write(conflict_fd , "C", strlen("C"));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}

			bytes = write(conflict_fd , " ", strlen(" "));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}

			bytes = write(conflict_fd , server_file_version, strlen(server_file_version));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}
			...
			bytes = write(conflict_fd,server_file_path,strlen(server_file_path));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}
			...
			bytes = write(conflict_fd,livehash,strlen(livehash));
			if(bytes == -1){
				printf("Failed to write to upadate file!\n");
				return -1;
			}
	...
	} // if statements that checks if client hash and server hash and live hash are equal

		//freer all variables since and break from while loop since we found the path and checcked for modifies or conflicts
		free(client_tag);
		free(client_file_version);
		free(client_file_path); 
		free(client_hash);
		...
		break;
		}//end of if statement that determines if paths are the same

		//if paths arent the same then we have to free client data to get ready to compare the next entry with the servers data
		free(client_tag);
		free(client_file_version);
		free(client_file_path); 
		free(client_hash);
		...
		}/////end of client hash ==NULL;


		}//strcmp to find space and new line
		else{
		...
		}//while loop that goes through clinet manifest

		free(server_file_version);
		free(server_file_path);
		free(server_hash);
		server_file_version = NULL;
		server_file_path = NULL;
		server_hash = NULL;
		lseek(CFD , 0,SEEK_SET);//lseek to the start of the file
		
/////////////////////////////////////////////////////////////////////////////////////////////////////////end of traverse client manifest for match to find conflict or modify///////////////////////////////////////////////////////////////////////


		}// end of server hash if statment


	}
	else{

		//we are building the buffer word
		strcat(server_word,server_buffer);
...
//now go through client manifest and find filepaths that are new or removed and add them accordingly to the update file

int good = add_entries(CFD, head,Hhead,Updatepath,Vhead);
if(good == -1){

	printf("Update method failed!\n");
	return -1;
}
...
int Upate_Method( int socketfd, int Cfd, char *Project_name){
...
bytes = read(socketfd , Buffer, 1); // this reads the string passed in by the server
...
//check to see if manifest versions are the same


char * Updatepath = (char *) malloc(strlen("./")+strlen(Project_name)+ +strlen("/.Update")+1);//allocated memory for .update file to be created
char * conflict_path = (char *) malloc(strlen("./")+strlen(Project_name)+ +strlen("/.Conflict")+1);
//making project path
strcat(Updatepath ,"./");
strcat(Updatepath, Project_name);
...
if(strcmp(VersionClient,VersionServer)== 0 ){
	//meaning that the versions are the same so make an empty .Update file
	int tryopen = open(Updatepath,O_RDONLY);
	int tryc =open(conflict_path,O_RDONLY);
...
if(tryopen!= -1){
	...
	if(madeU == -1){
		...
		// need to remove .Conflict file///65465436543654653465436454ewdfsadfgfdgfdgfdg
		
		if(tryc!= -1){
			close(tryc);
			int status = remove(conflict_path);//Project file has path to file
					if(status == -1){
	...
	//return 1 cause no changes are made to the manifest file of the client
	return 1;
}
if(tryc!= -1){
			close(tryc);
...
return 1;
	...
	//versions are not the same so we  have to find the differences in the manifests
	int work =Compare_manifestsU( socketfd, cfd, charsinfile, size,  bytesread, Project_name);
	if(work == -1){
...
char * readtocolon(int fd){
	...
	while(strcmp(Buffer,":")!=0){
		...
		if(bytes== -1){
			printf("Could not read from server or File!\n");
		}
...
int PushtoServer(int fd,int sockfd){
	int eof = 10;
	char * Buffer =(char *) malloc(sizeof(char));
	char* Word =(char *) malloc(sizeof(char));
	char* FilePath =NULL;
	...
	while(eof!=0){
		...
		if(strcmp(Word,"\n")== 0 && tag == 1 ){
			//need to open file and add to stream

			//sending over size of file path
			int String_length = strlen(FilePath);
			char * FileL = (char *) malloc(strlen(FilePath));
			// sprintf(FileL,"%d", String_length);
			// sendtoServer(sockfd,FileL);
			// sendtoServer(sockfd,":");

			// //now send file path
			// sendtoServer(sockfd,FilePath);
			// sendtoServer(sockfd,":");
			//now send contents of file 
			free(FileL);
			FileL= NULL;
		...
		int sizeto;
		if(stat(FilePath,&std)== 0){

			sizeto = (int)std.st_size;
		}

	
			 FileL = (char *) malloc(sizeto *sizeof(char));
			sprintf(FileL,"%d", sizeto);
			// send over size of bytes for file
			sendtoServer(sockfd,FileL);
			sendtoServer(sockfd,":");
			free(FileL);
			FileL= NULL;
			FileL = (char *) malloc(sizeto *sizeof(char));
			int bytes = read(Cfd, FileL ,sizeto);
		...
		//we have reached the end of the entry and do not need to add this to the stream
		free(Word);
		 Word =NULL;
		char* Word =(char *) malloc(sizeof(char));
		continue;
	...
	if(tag == 0){
		...
		if(strcmp(Word ,"A") == 0 || strcmp(Word, "M") == 0){
		tag  =1;//this means that we need to send over the files that are added or modified
		free(Word);
		 Word =NULL;
		char* Word =(char *) malloc(sizeof(char));
		continue;
			...
			if(version == 0){
				...
				path++;
				FilePath =(char *) malloc(strlen(Word)+1);
				strcpy(FilePath,Word);//copies file path to Word
				free(Word);
				Word =NULL;
				 char* Word =(char *) malloc(sizeof(char));
				 continue;
			...
			//we do not need the word since it is an entry that does not need to be sent over
			free(Word);
			Word =NULL;
			char* Word =(char *) malloc(sizeof(char));
...
int sendFile(int fd ,int sockfd ,char * File_path){
		...
		char * charsname = (char *) malloc(sizeof(char) *commit_l + strlen(":")+1);
		...
		free(charsname);
		charsname =NULL;
		//bytes in commit files

		int digit =floor (log10 (abs (sizeto))) + 1;
		 charsname = (char *) malloc(sizeof(char) *digit+strlen(":")+1);
...
int updateManifest(int fd,  const char *md5 ,char * FilePath, char * ManiPath){
...
int size =st.st_size;
printf("hererererererer %d\n\n\n\n\n\n", size);
printf("path:%s\n\n\n\n\n\n" , FilePath);



if(size == 2){
printf("hererererererererererererererererere\n\n\n\n\n\n");
...
printf("here on some shit\n\n");
int update = updateManifest(pfd,md5,Full_path, Project_Path);
printf("here on some shit\n\n");
...
///////////////////////////////////////add///////////////////////////////////////////////////////////////////////////////////////////////
		...
		*/
		...
		if(argc>4){
			printf("Too many arguements, remove command failed!\n");
			return -1;
		}
		else if(argc<4){
			printf("Too few arguements, remove command failed!\n");
			return -1;
		}
	...
	*/
		...
		if(argc<3){
			printf("Too few arguements checkout command failed\n");
			return -1;
		}
		else if(argc>3){
			printf("Too mant arguements checkout command failed\n");
			return -1;
		}
...
if(argc<3){
	...
	if(dir){
		...
		int sent = sendFile(fd,pass, CommitFile_Path); //sends commit file
		int pushtoserver =PushtoServer(fd ,pass);
		if(pushtoserver == -1){
			return -1;
		}

		//send contents of file to 
...
//////////////////////// these if statements to see if the push command succeeded
	...
	update
		...
		1)gets the servers manifest file and compares it to the client manifest file to see if there are any changes from the server copy
		2)if there are then the client should make a .update file , for any chanages you should write into the .Update added, change,or removed entries with codes
		3)print to STDOUT in a similar fashion to commit, put entire entries in into .Update file 
		4)
	*/
if(argc<3){
...
//this will call the findconfig method which returns and int of 1 if found or  -1 if not found
		...
		*/
		...
		strcpy(project_name,argv[i+1]);
		char * manifestFile_Path = (char *) malloc(strlen("./")+strlen(project_name)+strlen("/.Manifest"));
		strcat(manifestFile_Path ,"/");
		strcat(manifestFile_Path , project_name);
		strcat(manifestFile_Path , "/.Manifest");
		strcat(project_name,":");
		int fd = open(manifestFile_Path,O_RDONLY);
		if(fd == -1){
		...
		int  worked =Upate_Method( pass, fd, argv[1+1]);
		if(worked == -1){
			printf("The update command has failed!\n");
			return -1;
		}
		else if(worked == 1){

			printf("The update command was successful!\n");
			return 0;
		}
	...
	if(argc<3){
			printf("Too few arguements upgrade command failed\n");
			return -1;
		}
		else if(argc>3){
			printf("Too many arguements upgrade command failed\n");
			return -1;
		}
		...
		*/
		...
		strcpy(project_name,argv[i+1]);
		char * updatePath = (char *) malloc(strlen("./")+strlen(project_name)+strlen("/.Conflict"));
		char * conflict = (char *) malloc(strlen("./")+strlen(project_name)+strlen("/.Conflict"));
